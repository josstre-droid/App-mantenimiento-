...
import {
  dbSelect, dbInsert, dbUpdate, dbDelete, uploadFoto, getSignedUrl, deleteFoto,
} from "../lib/api";
...
export default function ReportesFotograficos({ session, onBack, trabajoIdInicial }) {
  ...
  const cargarPartidas = async (tId, clienteIdOverride) => {
    setLoading(true);
    setError("");
    try {
      const clienteId = clienteIdOverride || trabajos.find((t) => t.id === tId)?.cliente_id;
      if (clienteId) {
        const eqs = await dbSelect(accessToken, "equipos", `select=id,tipo_equipo,numero_identificador&cliente_id=eq.${clienteId}&order=tipo_equipo.asc`);
  ...
  const crearTrabajo = async () => {
    ...
    try {
      ...
      setNuevoFolio("");
      setNuevoClienteId("");
      cargarPartidas(t.id, nuevoClienteId);
    } catch (e) {
      setError(e.message);
    }
  };
  ...
  const vincularEquipo = async (partidaId, equipoId) => {
    setPartidas((prev) => prev.map((p) => (p.id === partidaId ? { ...p, equipo_id: equipoId || null } : p)));
    try {
      await dbUpdate(accessToken, "partidas", `id=eq.${partidaId}`, { equipo_id: equipoId || null });
    } catch (e) {
      setError(e.message);
    }
  };
  ...
  const subirFoto = async (partidaId, fileList) => {
    for (const file of Array.from(fileList)) {
      try {
        ...
        if (partidaActual?.equipo_id) {
          await dbUpdate(accessToken, "equipos", `id=eq.${partidaActual.equipo_id}`, {
            fecha_ultimo_mantenimiento: new Date().toISOString().slice(0, 10),
          });
        }
      } catch (e) {
        setError(e.message);
  ...
  const borrarTrabajo = async () => {
    const ok = window.confirm(
      `¿Borrar por completo el trabajo ${trabajo?.folio}? Esto elimina el reporte fotográfico (fotos y firmas) y también la cotización de este trabajo, si tiene una. Esta acción no se puede deshacer.`
    );
    if (!ok) return;
    try {
      const rutas = partidas.flatMap((p) => p.fotos.map((f) => f.url_foto));
      await Promise.all(rutas.map((r) => deleteFoto(accessToken, r).catch(() => {})));
        ...
        <>
          {partidas.map((partida, idx) => (
            <PartidaCard
              ...
              onEquipo={(v) => vincularEquipo(partida.id, v)}
          ...
          {esAdmin && (
            <button onClick={borrarTrabajo} style={{ ...secondaryBtnStyle, width: "100%", justifyContent: "center", padding: 12, marginTop: 10, color: "#B5482F", borderColor: "#F0C9C2" }}>
              <Trash2 size={14} /> Borrar trabajo completo (reporte + cotización)
            </button>
          )}
        </>
      )}
...
function PartidaCard({ partida, idx, puedeEditar, puedeFirmar, onDescripcion, onEquipo, equiposDisponibles, onFotos, onComentario, onFirmaDibujada, onFirmaTexto }) {
  return (
    <div style={cardStyle}>
      ...
      <div style={{ padding: 16 }}>
        ...
        {puedeEditar && equiposDisponibles?.length > 0 && (
          <select
            value={partida.equipo_id || ""}
            onChange={(e) => onEquipo(e.target.value)}
            style={{ ...inputStyle, marginBottom: 14, fontSize: 12.5, color: "#8A5A2E" }}
          >
            <option value="">Vincular a un equipo (opcional)</option>
            {equiposDisponibles.map((eq) => (
              <option key={eq.id} value={eq.id}>{eq.tipo_equipo} — {eq.numero_identificador}</option>
            ))}
          </select>
        )}
