...
import { ArrowLeft, Plus, Trash2, Loader2, ChevronDown, Printer } from "lucide-react";import { dbSelect, dbInsert, dbUpdate, dbDelete } from "../lib/api";
...
export default function Cotizaciones({ session, onBack }) {
    ...
    (async () => {
      try {
        ...
        const ts = await dbSelect(
          accessToken,
          "trabajos",
          "select=*,clientes(nombre),cotizaciones(estatus,folio,created_at)&order=created_at.desc&cotizaciones.order=created_at.desc&cotizaciones.limit=1"
        );
        setTrabajos(ts);
      } catch (e) {
        setError(e.message);
  ...
  const agregarPartida = async () => {
    ...
    try {
      const [p] = await dbInsert(accessToken, "partidas", {
        ...
        descripcion: "", detalle: "", cantidad: 1, unidad: "PZA", precio_unitario: 0,
      });
  ...
  const borrarCotizacion = async () => {
    if (!window.confirm(`¿Borrar la cotización ${cotizacion.folio}? Los conceptos y fotos del reporte fotográfico de este trabajo NO se borran, solo se desvinculan de esta cotización.`)) {
      return;
    }
    try {
      await dbDelete(accessToken, "cotizaciones", `id=eq.${cotizacion.id}`);
      setTrabajos((prev) => prev.map((t) => (t.id === trabajoId ? { ...t, cotizaciones: [] } : t)));
      setCotizacion(null);
      setPartidas((prev) => prev.map((p) => ({ ...p, cotizacion_id: null })));
    } catch (e) {
      setError(e.message);
    }
  };
    ...
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "24px 20px 60px" }}>
      ...
      {loading ? (
        ...
        <>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center", fontSize: 12.5, color: "#6B6656", marginTop: 4 }}>
                      {puedeEditar ? (
                        ...
                        <span>{p.cantidad} {p.unidad} × {money(p.precio_unitario)} = <b>{money(p.cantidad * p.precio_unitario)}</b></span>
                      )}
                    </div>
                  </div>
                  {puedeEditar && (
          ...
          {puedeEditar && (
            <button onClick={borrarCotizacion} className="no-print" style={{ ...secondaryBtnStyle, marginTop: 10, color: "#B5482F", borderColor: "#F0C9C2" }}>
              <Trash2 size={14} /> Borrar cotización
            </button>
          )}
        </>
      )}
