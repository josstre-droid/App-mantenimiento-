...
import { Wrench, LogOut, FileText, Camera, DollarSign, Receipt, Loader2, AlertCircle } from "lucide-react";

const SUPABASE_URL = "https://iwxstzjdcuuptkncexod.supabase.co";
...
async function getCliente(accessToken, clienteId) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/clientes?id=eq.${clienteId}&select=*`,
    {
      headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}` },
    }
  );
...
const ROL_COLOR = { admin: "#E0983A", tecnico: "#3F7268", cliente: "#5A6B8C" };
      ...
      <div style={{ maxWidth: 760, margin: "28px auto", padding: "0 20px" }}>
        ...
        >
          {modulos.map((m, i) => (
            ...
            >
              <m.icon size={20} color="#E0983A" />
              <span style={{ fontWeight: 600, fontSize: 14 }}>{m.label}</span>
              <span style={{ fontSize: 11.5, color: "#9C9585" }}>Próximamente</span>
            </div>
          ))}
        </div>
      </div>
